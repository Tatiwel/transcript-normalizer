...
RE_UNIDADE = re.compile(r'(\d[\d.,]*)\s+(B|bit|be)\b(?![\w])')
RE_UNIDADE2 = re.compile(r'(\d[\d.,]*)\s+(\d)\s+(B|bit|be)\b')  # '6.7 7 B'
RE_REAIS = re.compile(r'(bilh[oõ]es|milh[oõ]es)\s+deais\b')
for l in linhas:
    ts, txt = l.strip().split(' ', 1)
    for m in RE_UNIDADE2.finditer(txt): propostas[ts].append((m.group(0), 'bi', 100))
    for m in RE_UNIDADE.finditer(txt):
        if not RE_UNIDADE2.search(txt): propostas[ts].append((m.group(0), 'bi', 100))
    for m in RE_REAIS.finditer(txt): propostas[ts].append((m.group(0), 'reais', 100))
    ...
    for n in (1, 2, 3):
        for i in range(len(pal) - n + 1):
            ...
            melhor = max(((fuzz.ratio(tn, c) if len(tn)>=6 and len(c)>=6 and abs(len(tn)-len(c))<=2 else (100 if tn==c else 0), termo, c) for c, termo in cands), default=(0, None, None))
