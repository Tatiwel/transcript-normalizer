...
- The user declares a **domain pack**: a list of terms, each with aliases, observed misrecognitions (*variantes*), and collocations. The tool never infers the domain.
...
- `fixtures/R2Qgz8tFWVI/`: one Brazilian Portuguese finance video (31 min). `legenda.txt` is the raw platform caption, `gabarito.csv` is a hand-checked gold list of 168 domain-term errors plus 2 lines that must not be touched, `termos.yaml` is the first domain pack.
